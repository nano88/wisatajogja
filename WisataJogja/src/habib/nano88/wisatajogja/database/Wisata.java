...
public class Wisata extends SQLiteOpenHelper {
	...
	public boolean editWisata(Lokasi wisata, int idLokasi) {
		...
		return ((db.update(TB_WISATA, values, COLUMN.idLokasi + "=?",
				new String[] { String.valueOf(idLokasi) })) == 1) ? true
				: false;
	}

	public boolean deleteWisata(int idLokasi) {
		return ((db.delete(TB_WISATA, COLUMN.idLokasi + "=?",
				new String[] { String.valueOf(idLokasi) })) == 1) ? true
				: false;
	}
}